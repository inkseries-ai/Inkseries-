...
app.get("/api/novels/:slug", async (c) => {
  ...
  const nextEpisode = processedChapters.find((ch: Record<string, unknown>) => ch.is_locked);
...
app.get("/api/events/reminders", async (c) => {
  ...
  return c.json({ reminders: (reminders.results || []).map((r: Record<string, unknown>) => r.event_id) });
});
